...
- 自动回收中断超时任务，分阶段清理内存
- 持久化 session ID，服务重启后可恢复对话
- **异步任务执行** — 立即返回 202，任务完成后回调到发起会话
- **任务注册表持久化** — 任务写入 `~/.autocli/.tasks/`，重启后自动恢复 queued/submitted 任务
- **失败自动重试** — 任务失败时自动重试，最多 `maxRetry` 次（默认 3 次）
- **Git Worktree 隔离** — 代码任务在独立分支/worktree 中运行（可选开启）
- **定时监控** — 每 10 分钟检测超时任务并触发重试
- 提供 React 前端面板用于监控和操作
...
session 映射持久化到 `~/.autocli/`，重启后用同一 `sessionKey` 自动恢复对话：
...
对话历史（role/content）同样持久化到 `~/.autocli/.sessions/<sessionId>.json`。
...
| `OPENCLAW_GATEWAY_TOKEN` | — | Gateway 认证 token |
| `AUTOCLI_BASE_URL` | `http://127.0.0.1:8700` | autocli 技能调用的后端地址 |
| `AUTOCLI_WORKSPACE_ROOT` | `~/.autocli` | 所有持久化数据的根目录 |
| `AUTOCLI_PROJECT_NAME` | 当前目录名 | 项目名，工作空间路径：`$ROOT/$PROJECT_NAME` |
| `AUTOCLI_GIT_ROOT` | `process.cwd()` | Git 仓库根目录（用于 worktree 隔离） |
| `AUTOCLI_USE_WORKTREE` | `0` | 设为 `1` 默认启用 git worktree 隔离 |
| `AUTOCLI_MONITOR_INTERVAL_MS` | `600000` | 任务监控间隔（10分钟） |
| `AUTOCLI_TASK_TIMEOUT_MS` | `1800000` | 任务超时自动失败时长（30分钟） |
| `OPENCLAW_TASK_RETENTION_MS` | `900000` | completed/failed 保留时长（15分钟） |
| `OPENCLAW_TERMINATED_RETENTION_MS` | `18000000` | terminated 保留时长（5小时） |
| `OPENCLAW_INTERRUPT_TTL_MS` | `28800000` | waiting_input 超时时长（8小时） |
